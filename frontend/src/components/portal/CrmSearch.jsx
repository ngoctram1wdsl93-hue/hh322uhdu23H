...
export default function CrmSearch() {
  ...
  const ResultList = ({ onPick }) => (
    results.length === 0 ? (
      ...
      <ul className="max-h-[60vh] overflow-y-auto py-1" data-testid="crm-search-results">
        {results.map((it, i) => (
          <li key={it.path}>
            <button
              type="button"
              ...
              onClick={() => onPick(it)}
              className={`flex w-full items-center justify-between gap-3 px-4 py-2.5 text-left transition-colors ${i === active ? "bg-[#F4FBEF]" : "hover:bg-slate-50"}`}
  ...
  return (
    <>
      ...
      <div className="relative hidden sm:block" ref={boxRef} data-testid="crm-search">
        ...
        {open && q && (
          <div className="absolute left-0 right-0 z-50 mt-2 overflow-hidden rounded-2xl border border-[#0B1A14]/10 bg-white shadow-xl" data-testid="crm-search-panel">
