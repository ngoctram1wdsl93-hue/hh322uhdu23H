...
async def manager_offline_sign_contract(
    ...
    from server import _STATIC_DIR  # lazy import to avoid circular import
    sdir = pathlib.Path(_STATIC_DIR) / "signed-contracts"
    sdir.mkdir(parents=True, exist_ok=True)
    out_name = f"signed_{invoice_id}_{int(datetime.now(timezone.utc).timestamp()*1000)}.{ext}"
    with open(sdir / out_name, "wb") as f:
        f.write(content)
    url = f"/api/static/signed-contracts/{out_name}"
