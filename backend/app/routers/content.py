...
DEFAULT_SITE_INFO: Dict[str, Any] = {
    ...
    "footer": {
        "contacts": {
            "phones": ["+380 44 333 44 55", "+380 50 123 45 67"],
            "email": "info@eco-utyl.ua",
            "addresses": [
                "Україна, м. Київ, вул. Екологічна, 1",
                "Україна, м. Київ, вул. Промислова, 24",
            ],
            "working_hours": "Пн - Пт, 09.00 - 18.00",
            "registration_address": "Україна, 01001, м. Київ, вул. Хрещатик, 1",
        },
        "socials": {
            ...
            "viber":     {"enabled": True,  "url": "viber://chat?number=%2B380443334455"},
        },
        "viber_community": {
            "enabled": True,
            "url": "viber://chat?number=%2B380443334455",
    ...
    "partners": {
        "enabled": True,
        "title_uk": "Наші партнери",
        "title_en": "Our partners",
        "subtitle_uk": "Компанії та організації, що довіряють ECO.NOVA",
        "subtitle_en": "Companies and organisations that trust ECO.NOVA",
        "items": [],
    },
    "updated_at": None,
    "updated_by": None,
...
async def _get_site_info_doc():
    ...
    for k in ("policies", "footer", "cookie_banner", "header", "faq", "reviews", "before_after", "hero", "partners"):
...
async def update_site_info_admin(
    ...
    update = {}
    for key in ("policies", "footer", "cookie_banner", "header", "faq", "reviews", "before_after", "hero", "partners"):
...
async def upload_partner_logo_admin(
    ...
    dest = partners_dir / fname
    with open(dest, "wb") as f:
        f.write(content)

    url = f"/api/static/partners/{fname}"
