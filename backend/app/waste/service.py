...
async def price_estimate(
    ...
    if rule:
        ...
        container_per_kg = _num(rule.get("containerPerKg"), CONTAINER_FEE_PER_KG)
        transport_flat = _num(rule.get("transportFlat"), TRANSPORT_BASE)
        transport_per_kg = _num(rule.get("transportPerKg"), TRANSPORT_PER_KG)
        urgent_pct = _num(rule.get("urgentSurchargePct"), URGENT_SURCHARGE)
        ...
        min_charge = 0.0
        container_per_kg = CONTAINER_FEE_PER_KG
        transport_flat = TRANSPORT_BASE
        transport_per_kg = TRANSPORT_PER_KG
        urgent_pct = URGENT_SURCHARGE
        apply_region = True
        source = "default"
        applied_rule = None
