...
async def list_categories(accepted: Optional[bool] = None):
    ...
    for key, meta in CATEGORIES.items():
        cnt = await db[S.C_CODES].count_documents({**base_q, "category": key})
        haz = await db[S.C_CODES].count_documents({**base_q, "category": key, "hazardous": True})
        # Приховувати порожні категорії на публічному фронті (accepted-only).
        if accepted and cnt == 0:
            continue
        out.append({
            "key": key,
            "name": meta["name"],
...
async def pricing_meta():
    ...
    return {
        ...
        "defaults": {
            "urgent_surcharge_pct": S.URGENT_SURCHARGE,
            "container_fee_per_kg": S.CONTAINER_FEE_PER_KG,
            "transport_base": S.TRANSPORT_BASE,
            "transport_per_kg": S.TRANSPORT_PER_KG,
        },
        "currency": "UAH",
    }
