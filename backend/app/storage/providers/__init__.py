...
def get_storage_provider() -> StorageProvider:
    ...
    if _singleton is not None:
        return _singleton
    kind = (os.environ.get("STORAGE_PROVIDER") or "local").strip().lower()
    try:
        if kind == "minio":
        ...
        logger.warning("[storage] provider '%s' init failed (%s); falling back to local", kind, e)
        _singleton = LocalStorageProvider()
    logger.info("[storage] active provider: %s", _singleton.name)
    return _singleton


__all__ = ["StorageProvider", "StoredObject", "LocalStorageProvider", "MinioProvider", "R2Provider", "get_storage_provider"]
