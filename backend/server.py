...
async def _seed_demo_client_from_env():
    ...
    """
    email = (os.environ.get("BIBI_CLIENT_EMAIL") or "").strip().lower()
    pwd = os.environ.get("BIBI_CLIENT_PASSWORD") or ""
    if not email or not pwd:
        return  # not configured → skip (prod-safe)

    name = (os.environ.get("BIBI_CLIENT_NAME") or "Demo Client").strip()
    company_name = (os.environ.get("BIBI_CLIENT_COMPANY") or "ТОВ «Демо Клієнт»").strip()
...
async def update_reminder_settings(payload: Dict[str, Any] = Body(default={})):
    ...
    for field, (lo, hi) in _REMINDER_INT_FIELDS.items():
